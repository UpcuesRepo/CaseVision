...
$(document).ready((function(){function e(e,t){let r=e.querySelector(".drop-zone__thumb");if(e.querySelector(".drop-zone__prompt")&&e.querySelector(".drop-zone__prompt").remove(),r||(r=document.createElement("div"),r.classList.add("drop-zone__thumb"),e.appendChild(r)),r.dataset.label=t.name,t.type.startsWith("image/")){const e=new FileReader;e.readAsDataURL(t),e.onload=()=>{r.style.backgroundImage=`url('${e.result}')`}}else r.style.backgroundImage=null}$(".add-after").after('<svg width="10" height="10" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8.33366 2.5L3.75033 7.08333L1.66699 5" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>'),$(".selectpicker").selectpicker(),$("#file-upload").change((function(){var e=this.value.match(/([^\/\\]+)$/)[1];$("#filename").text(e)})),document.querySelectorAll(".drop-zone__input").forEach((t=>{const r=t.closest(".drop-zone");r.addEventListener("click",(e=>{t.click()})),t.addEventListener("change",(o=>{t.files.length&&e(r,t.files[0])})),r.addEventListener("dragover",(e=>{e.preventDefault(),r.classList.add("drop-zone--over")})),["dragleave","dragend"].forEach((e=>{r.addEventListener(e,(e=>{r.classList.remove("drop-zone--over")}))})),r.addEventListener("drop",(o=>{o.preventDefault(),o.dataTransfer.files.length&&(t.files=o.dataTransfer.files,e(r,o.dataTransfer.files[0])),r.classList.remove("drop-zone--over")}))}));[...document.querySelectorAll('[data-bs-toggle="tooltip"]')].map((e=>new bootstrap.Tooltip(e)))}));